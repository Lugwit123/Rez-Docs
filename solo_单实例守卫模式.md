...
状态：独立机制文档，截至 2026-09-17。源码热重载的现行实现、`L_SRC_WATCH` 门控及服务侧重启约定见[src_hot_reload_源码热重载与主页常驻.md](src_hot_reload_源码热重载与主页常驻.md)；本文只保留与 `.solo` / `.soloignore` 单实例裁决直接相关内容。
